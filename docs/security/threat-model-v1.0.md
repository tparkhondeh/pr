...
| T-26 | دورزدن Cost/Consent/Eval توسط Provider، Retry تکراری، خروجی خارج Schema یا نسبت‌دادن هزینه ساختگی | P0 | Registry نسخه‌دار بر Purpose+Schema، Rollout و Eval fail-closed، Tenant/Owner binding، Data Class allowlist، رضایت پردازش بیرونی، Model Input Safety قبل از Side Effect، Golden Safety Eval چندزبانه در CI، Reservation اجباری، Timeout/Abort، Durable Invocation Journal metadata-only با RLS و terminal transition واحد، Charge پیش از Output validation و `unmetered` برای قیمت نامعلوم | Provider واقعی تا فعال‌شدن Journal PostgreSQL در Production، Safety eval روی Corpus ناشناس‌شده، billing reconciliation، crash/retry drill، canary rollback و تأیید مالک غیرفعال می‌ماند |
