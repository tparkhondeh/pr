...
| T-05 | ادعای عمومی ساختگی یا unsupported | P0 | evidence-bound Claim Registry، تطابق دقیق ID/Excerpt/Body، اسکن مستقل Claim حساس پس از حذف Statementهای متصل، Human approval و `authentic-execution-eval-v1` با پنج حمله و Approval صفر | Golden Set سبز است؛ Provider واقعی نیازمند Corpus، Claim extraction و red-team مستقل است |
| T-11 | مخلوط‌شدن Research بیرونی با Personal Memory، Citation کهنه/متعارض یا SSRF/DNS rebinding در Connector | P0 | Research store جدا، Claim پیشنهادی، freshness/conflict gate، `research-source-safety-v1` با URL عمومی، DNS همه-عمومی، address pinning، redirect revalidation، response limits و `external-intelligence-eval-v1` با ۱۵/۱۵ حمله مسدود | Fetch خودکار وب خاموش است؛ هر Adapter واقعی باید resolver/connect pinning، corpus اختصاصی و revocation drill خود را اضافه کند |
...
| T-21 | تبدیل Narrative Candidate به Brand Fact، استفاده از Asset بدون مجوز، یا اعتماد کاذب به Anti-Generic/Voice heuristic به‌عنوان تضمین اصالت | P0 | فقط Asset دارای `brandUsage`، Seed با epistemic type و maturity محدود، Fail-closed برای Ref نامعتبر، Findingهای مستقل، مرز صریح Fact/Claim/Publish Approval، `externalActionPermitted=false` و شش Authenticity Case بدون Raw Asset leakage | Golden Set چندزبانه فعال است؛ پیش از Model-based narrative inference، Corpus/Adversarial Paraphrase بزرگ‌تر، permission re-check و Human authenticity calibration الزامی است |
| T-22 | یکی‌گرفتن Trend با Opportunity، ساخت Filter Bubble یا تبدیل Source کهنه/متعارض به Action | P0 | پنج عامل مستقل و توضیح‌پذیر بدون Score پنهان، stale/unverified=ignore، conflict=monitor، سقف یک Exploration، مرز قطعی Strategy Review از Action/Approval و Golden Gate اطلاعات بیرونی بدون Side Effect | پیش از News/Social/Media connector، provenance/allowlist اختصاصی، rate limit، attention budget، Human review و connector revocation test الزامی است |
