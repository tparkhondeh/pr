...
import { rememberedLifetime, rememberedSessionStore, type OwnerSession } from './remembered-sessions.js';
...
export function createOwnerSessionGate(options: Options) {
  ...
  return async (request: IncomingMessage, response: ServerResponse): Promise<boolean> => {
    ...
    let version: string;
    try { version = digest(options.version()); } catch { response.writeHead(503); response.end('ورود موقتاً در دسترس نیست.'); return true; }
    let pruned = false;
    for (const [id, session] of sessions) {
      if (now() < session.created || now() - session.created >= (session.remembered ? rememberedLifetime : 8 * 3600_000) ||
        (!session.remembered && now() - session.seen >= 30 * 60_000) || session.version !== version) {
        sessions.delete(id); pruned = true;
      }
    }
    if (pruned) persist();
    ...
    if (path === '/login' || path === '/logout') {
      ...
      if (path === '/logout') {
        // All-device revocation requires a valid browser session as well as same-origin POST.
        if (request.headers['x-pr-revoke-all'] === '1') {
          if (!session) { response.writeHead(401); response.end(); return true; }
          sessions.clear();
        } else sessions.delete(key);
        persist(); response.setHeader('set-cookie', cookie('', 0)); redirect(response, '/login'); return true;
      }
      ...
      if (digest(options.version()) !== version) { response.writeHead(401); response.end(); return true; }
      sessions.delete(key);
      if (sessions.size >= 64) sessions.delete(sessions.keys().next().value ?? '');
      const next = randomBytes(32).toString('hex');
      const remembered = form.get('remember') === 'yes';
      sessions.set(digest(next), { created: now(), seen: now(), version, remembered });
      try { persist(); } catch { sessions.delete(digest(next)); response.writeHead(503); response.end('ذخیرهٔ ورود ممکن نشد؛ دوباره تلاش کنید.'); return true; }
      response.setHeader('set-cookie', cookie(next, remembered ? rememberedLifetime / 1000 : 8 * 3600)); redirect(response, '/'); return true;
    }
    if (path === '/health' || path === '/ready') return false;
    // This exact licensed font is public for the login form; no application assets are opened.
    if (path === '/fonts/Vazirmatn-33.003.woff2' && (request.method === 'GET' || request.method === 'HEAD')) return false;
...
function loginPage(logout: boolean, failed = false): string {
  return `<!doctype html><html lang="fa" dir="rtl"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${logout ? 'خروج امن' : 'ورود'} | PR</title><style>body{margin:0;background:#f1f5f9;color:#172b3a;font-family:Vazirmatn,Tahoma,Arial,sans-serif;display:grid;place-items:center;min-height:100dvh}main{background:white;padding:32px;border-radius:20px;box-shadow:0 16px 60px #18384a18;width:min(360px,80vw)}h1{font-size:26px}p{line-height:1.9;color:#486070}label{display:block;margin:20px 0 8px}input,button{box-sizing:border-box;width:100%;font:inherit;padding:13px;border-radius:9px;border:1px solid #9aabb5}input{direction:ltr;text-align:left}input[type=checkbox]{width:auto;margin-left:8px}label.choice{font-size:14px;line-height:1.9}button{margin-top:24px;background:#146b61;color:white;border:0;cursor:pointer}a{display:block;margin-top:20px;color:#146b61}.error{color:#ac2430}small{display:block;margin-top:22px;color:#596c78;line-height:1.8}@font-face{font-family:Vazirmatn;src:url('/fonts/Vazirmatn-33.003.woff2') format('woff2');font-weight:100 900;font-style:normal;font-display:swap}*{box-sizing:border-box}html{font-size:100%}body{font-size:1rem;line-height:1.85;padding:1rem}main{width:min(28rem,100%);padding:clamp(1.25rem,4vw,2rem)}h1{font-size:1.75rem;line-height:1.6}p,label{font-size:1rem;overflow-wrap:anywhere}input,button{font-size:1rem;min-height:2.75rem}input[type=checkbox]{min-height:0;width:1.125rem;height:1.125rem;vertical-align:middle}label.choice,small{font-size:.9375rem;line-height:1.9;color:#40576a}button{font-weight:600;line-height:1.8}a{font-size:1rem}:focus-visible{outline:3px solid #a36616;outline-offset:3px}</style><main><span>WealthOS · PR</span><h1>${logout ? 'خروج از حساب' : 'خوش آمدید'}</h1><p>${logout ? 'با خروج، دسترسی این مرورگر به اطلاعات شما بسته می‌شود.' : 'برای ورود به فضای خصوصی برند شخصی خود، اطلاعات حساب را وارد کنید.'}</p>${failed ? '<p class="error" role="alert">نام کاربری یا رمز درست نیست. دوباره امتحان کنید.</p>' : ''}<form method="post" action="${logout ? '/logout' : '/login'}">${logout ? '<label class="choice"><input type="checkbox" name="revoke_all">خروج از همهٔ دستگاه‌ها و لغو ورودهای به‌خاطرسپرده‌شده</label>' : '<label for="username">نام کاربری</label><input id="username" name="username" autocomplete="username" required maxlength="100"><label for="password">رمز عبور</label><input id="password" name="password" type="password" autocomplete="current-password" required maxlength="512"><label class="choice"><input type="checkbox" name="remember" value="yes">مرا در این مرورگر به خاطر بسپار — ۱۴ روز</label>'}<button type="submit">${logout ? 'خروج امن' : 'ورود به PR'}</button></form>${logout ? '<a href="/">بازگشت به برنامه</a>' : '<small>روی دستگاه مشترک این گزینه را فعال نکنید. بدون آن، پس از ۳۰ دقیقه عدم استفاده ورود مجدد لازم است. ورود ماندگار پس از ۱۴ روز یا خروج لغو می‌شود؛ رمز در مرورگر ذخیره نمی‌شود.</small>'}</main></html>`;
}
