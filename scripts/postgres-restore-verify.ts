...
import { latestSchemaMigration } from '../src/database/postgres.js';
...
try {
  const migrations = await client.query<Readonly<{ count: string; latest: string | null }>>(
    'SELECT count(*)::text AS count, max(id) AS latest FROM public.pr_schema_migrations',
  );
  ...
  const expectedMigrationCount = readdirSync('db/migrations')
    ...
    .length;
  const migrationJournal = migrations.rows[0];
  if (
    !migrationJournal ||
    migrationJournal.count !== String(expectedMigrationCount) ||
    migrationJournal.latest !== latestSchemaMigration
  ) {
